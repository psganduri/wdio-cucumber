...
class AuthPage {
  ...
  createAccount = async (emailId, address) => {
    ...
    await this.input_address_City.setValue("Melbourne");
    ...
    await this.input_MobilePhone.setValue("+61 451256565");
    await this.input_AddressAlias.setValue("My Address");
    await this.btn_Submit.click();
    console.log("end of create user");
    // await addProductPage.btnTshirts.waitForDisplayed();
    // await addProductPage.navitageToTshirts();
    // await addProductPage.addProductToCart();
    // // await addProductPage.btnProceedToCkout();
    // await addProductPage.navigateToPaymentsPage();
    // await addProductPage.verifyProductDetailsInCart();
    // // browser.pause(20000);
  };
}
